...
srqflights %>% 
  filter(month == 1) %>% 
  group_by(carrier, flight, origin) %>% 
  summarise(avg_delay = mean(dep_delay)) # avg delay time for each category
...
srqflights %>% 
  filter(month == 1) %>% 
  group_by(carrier, flight, origin) %>% 
  summarise(avg_delay = mean(dep_delay, na.rm = TRUE)) # remove NAs

# we can change from mean to median
srqflights %>% 
  filter(month == 1) %>% 
  group_by(carrier, flight, origin) %>% 
