...
```{r}
# REORDER: reorder geography based on population estimate number 
...
DCA_delays = flights %>%
  ...
  filter(dep_delay >= 0) %>%
  group_by(carrier) %>%
  summarize(avg_delay = median(dep_delay, na.rm = TRUE))

ggplot(DCA_delays, aes(x = carrier, y = avg_delay)) + 
  geom_col(color = "#9ebcda", fill = "#9ebcda") +
  coord_flip() +
  scale_y_continuous(name = "Carrier", labels = scales::comma) +
  scale_x_discrete(name = "Delay Time") +
  labs(title = "Median Delay Time for Flights to DCA", 
       subtitle = "2013",
       caption = "Source: U.S. Census Bureau") +
  theme_fivethirtyeight()
```
